function [TrainingTime, TestingTime, TrainingAccuracy,...
    TestingAccuracy, predicted_class] = ELM(TrainingData_File, TestingData_File,...
    InputWeight, BiasofHiddenNeurons, Elm_Type, ActivationFunction)
...
%%%%%%%%%% Calculate training & testing classification accuracy
    ...
    predicted_class = zeros(size(TV.T, 2),1);
    for i = 1 : size(TV.T, 2)
        [x, label_index_expected]=max(TV.T(:,i));
        [x, label_index_actual]=max(TY(:,i));
        predicted_class(i) = label_index_actual;
        if label_index_actual~=label_index_expected
